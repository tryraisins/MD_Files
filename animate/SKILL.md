...
Apply `ui-quality-baseline` to any visible interface touched by the animation. Reuse the product's tokens and motion system, preserve stable control geometry, gate hover behavior by pointer capability, include reduced-motion behavior, and verify the result at narrow and touch widths. Do not add motion to satisfy a quota.
